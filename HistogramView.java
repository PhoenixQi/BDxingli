...
//    private String[] mText_X;// X轴坐标
    ...
    public void upDataTextForXY(LinkedHashMap data) {
        Object[] objects1 = data.values().toArray();
        int[] signal = new int[objects1.length];
        for (int i = 0; i < signal.length; i++) {
            signal[i] = (int) objects1[i];
        }
        if (signal.length!=0){
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mPaint_X.setColor(Color.DKGRAY);//深灰色
        mPaint_X.setStrokeWidth(3);//设置线宽

        mPaint_Y.setColor(Color.GRAY);//灰色

        mPaint_InsideLine.setColor(Color.LTGRAY);//浅灰色
...
//        int downWeight_Every = (width - 50) / mText_X.length;//X轴每个数据的间距
        ...
        //画X坐标轴-横轴
//        canvas.drawLine(0, height - 50, width, height - 50, mPaint_X);//使用当前颜色在点 (startX, startY) 和 (stopX, stopY) 之间画一条线,是以小窗口左上角为参照点
//        canvas.drawLine(0, 0, 20, 20, mPaint_X);
        //画Y坐标轴-数轴
...
//                canvas.drawText(mText_Y[i - 1], 50, leftHeight_Every * (i - 1) + 10, mPaint_Text);//从上往下画刻度
//            }
            //画矩形
            for (int i = 1; i < data.length + 1; i++) {
                ...
                rect.left = downWeight_Every * i - 15;
                rect.right = downWeight_Every * i + 15;
                rect.top = (height - 50 - (int) (data_Yx * leftHeight_Every));
                rect.bottom = height - 45;
                ...
                //画矩形上的信号强度
                canvas.drawText(String.valueOf(data[i - 1]), 50 + downWeight_Every * (i - 1), (height - 40 - (int) (data_Yx * leftHeight_Every)) - 15, mPaint_Text);
            }
        }
    }
